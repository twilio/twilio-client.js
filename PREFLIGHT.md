...
```js
{
   ...
   * This will emit any RTCWarnings or PreflightTest.Warnings encountered
   */
  "warnings": [...]
}
```
...
Raised whenever the [Connection](https://www.twilio.com/docs/voice/client/javascript/connection) encounters an RTCWarning, or Preflight encounters a warning. RTCWarnings come from [Connection.on('warning')](https://www.twilio.com/docs/voice/client/javascript/connection#onwarning-handlerwarningname) and uses the same API format, and `PreflightTest.Warning`s are warnings generated by preflight test.
