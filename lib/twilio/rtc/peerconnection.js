...
PeerConnection.prototype._reassignMasterOutput = function reassignMasterOutput(pc, masterId) {
  ...
  return masterOutput.audio.setSinkId(idToReplace).then(() => {
    ...
    self._logger.info('Could not reassign master output. Attempted to roll back.');
  });
};
...
PeerConnection.prototype._fallbackOnAddTrack = function fallbackOnAddTrack(pc, stream) {
  ...
  if (!setAudioSource(audio, stream)) {
    pc._logger.info('Error attaching stream to element.');
  }
...
PeerConnection.prototype._setupChannel = function() {
  ...
  this.version.pc.onsignalingstatechange = () => {
    ...
    this._logger.info(`signalingState is "${state}"`);
  ...
  pc.onconnectionstatechange = () => {
    this._logger.info(`pc.connectionState is "${pc.connectionState}"`);
  ...
  pc.onicecandidate =  event => {
    ...
    this._logger.info(`ICE Candidate: ${JSON.stringify(candidate)}`);
  };
  ...
  pc.onicegatheringstatechange = () => {
    ...
    this._logger.info(`pc.iceGatheringState is "${pc.iceGatheringState}"`);
    this.onicegatheringstatechange(state);
  };

  pc.oniceconnectionstatechange = () => {
    this._logger.info(`pc.iceConnectionState is "${pc.iceConnectionState}"`);
...
PeerConnection.prototype._setupRTCDtlsTransportListener = function() {
  ...
  const handler = () => {
    this._logger.info(`dtlsTransportState is "${dtlsTransport.state}"`);
...
PeerConnection.prototype.iceRestart = function() {
  ...
  this._logger.info('Attempting to restart ICE...');
  this.version.createOffer(this.options.maxAverageBitrate, this.codecPreferences, { iceRestart: true }).then(() => {
    ...
    this._onAnswerOrRinging = payload => {
      ...
      if (this.status !== 'closed') {
        this.version.processAnswer(this.codecPreferences, sdp, null, err => {
          ...
          this._logger.info(`Failed to process answer during ICE Restart. Error: ${message}`);
        });
      }
    };

    this._onHangup = () => {
      this._logger.info('Received hangup during ICE Restart');
      this._removeReconnectionListeners();
    };
    ...
    const message = err && err.message ? err.message : err;
    this._logger.info(`Failed to createOffer during ICE Restart. Error: ${message}`);
...
PeerConnection.prototype.getOrCreateDTMFSender = function getOrCreateDTMFSender() {
  ...
  const pc = this.version.pc;
  if (!pc) {
    this._logger.info('No RTCPeerConnection available to call createDTMFSender on');
    return null;
  }
  ...
  if (typeof pc.getSenders === 'function' && (typeof RTCDTMFSender === 'function' || typeof RTCDtmfSender === 'function')) {
    ...
    if (chosenSender) {
      this._logger.info('Using RTCRtpSender#dtmf');
  ...
  if (typeof pc.createDTMFSender === 'function' && typeof pc.getLocalStreams === 'function') {
    ...
    if (!track) {
      this._logger.info('No local audio MediaStreamTrack available on the RTCPeerConnection to pass to createDTMFSender');
      return null;
    }

    this._logger.info('Creating RTCDTMFSender');
    this._dtmfSender = pc.createDTMFSender(track);
    return this._dtmfSender;
  }

  this._logger.info('RTCPeerConnection does not support RTCDTMFSender');
