...
PeerConnection.prototype._maybeSetIceAggressiveNomination = function(sdp) {
  return this.options.enableIceAggressiveNomination ? setIceAggressiveNomination(sdp) : sdp;
};
  ...
  this._onAnswerOrRinging = payload => {
    ...
    if (self.status !== 'closed') {
      self.version.processAnswer(
        ...
        this._maybeSetIceAggressiveNomination(payload.sdp),
        onAnswerSuccess,
        onAnswerError
      );
    }
...
PeerConnection.prototype.answerIncomingCall = function(callSid, sdp, rtcConstraints, rtcConfiguration, onMediaStarted) {
  ...
  this.version.processSDP(
    ...
    this.codecPreferences,
    this._maybeSetIceAggressiveNomination(sdp),
    { audio: true },
    onAnswerSuccess,
    onAnswerError
  );
