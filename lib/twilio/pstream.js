...
PStream.prototype.setToken = function(token) {
  ...
  const payload = {
    ...
    // NOTE (csantos): As of VSP 1.6, calls are not terminated after accidental disconnect (browser refresh).
    // Setting reconnectTimeout to zero terminates the call right away, preserving the old behavior in VSP 1.5
    reconnectTimeout: 0,
    token,
  };
