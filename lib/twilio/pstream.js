...
function PStream(token, uri, options) {
  ...
  this.addListener('close', () => {
    self._logger.info('Received "close" from server. Destroying PStream...');
    self._destroy();
  });
...
PStream.prototype.setToken = function(token) {
  this._logger.info('Setting token and publishing listen');
  this.token = token;
  const payload = {
    token,
...
PStream.prototype.destroy = function() {
  this._logger.info('PStream.destroy() called...');
  this._destroy();
  return this;
};
